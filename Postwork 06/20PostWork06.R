...
ventas sobre la base de los tres presupuestos de medios de comunicación. Ajuste modelos de regresión 
lineal múltiple a los datos advertisement.csv y elija el modelo más adecuado siguiendo los procedimientos
vistos
...
#Sesion-06/data/advertising.csv
...
# con base a la matriz anterior se observa la correlación fuerte entre Ventas y TV y una asociación positiva 
# entre Sales y las demás variables
...
#son efectos marginales los coeficientes como cambia la variable dependiente a variaciones unitarias eb la variable
#independiente
# por cada incremento unitario en TV las ventas se incrementan 0.054 por cada 
# se observa que Radio tiene un incremento unitario mayor, es decir por cada incremento de Redio se tiene
# 0.1 incremento en las ventas, es el dato más significativo
# se concluye que Newspaper no es significativo (valor 0.0003357)
# se tienen que establecer hipótesis para demostrar los efectos en la población, por lo tanto, se establecen las
# dos hipótesis:

#Ho:beta_i=0
#ha:beta_i!=0

# con base al análisis de los pValue y con un nivel de confianza del 95% con una significancia estadística de 0.05
#se tiene o concluye que Newspaper no ayuda a explicar el comportamiento de las ventas, (pvalue es de 0.954)
...
# ahora se va a revisar un nuevo modelo con una actualización del anterior por medio del comando update
# eliminando Newspaper
...
"
# se debe comparar el R2 ajustado para la comparación de los 2 modelos

# la diferencia es mínima entre ambos modelos (modelo1=0.9011 y modelo2=0.9016), aún así se puede tomar
# en cuenta el modelo 1
...
# Las gráficas muestran una alerta ya que los datos tienden a ser positivos concluyendo que se tiene un 
#mayor problema con TV

# a continuación se grafica para comparar los quantiles de una distribución  normal con los quantiles de los 
# residuos

qqnorm(resEstandarizados)
qqline(resEstandarizados)

# se puede concluir que los residuos se comportan como una normal, la mayor parte de puntos coincide 
#entre -1 y 1 de desviación estándar

# Lo que sigues es verificar si esto ocurre en la población
# para esto se aplica la prueba de shapiro wilk

shapiro.test(resEstandarizados)

# y las hipótesis a contrastar
...
"con el valor de pValue de 0.001365 se rechaza Ho por lo tanto no distribuye como una normal para una
significancia de 0.1, 0.05 y 0.01 y no permite hacer predicciones"
